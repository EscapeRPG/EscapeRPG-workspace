	...
	<body>
        ...
        <?php include $_SERVER['DOCUMENT_ROOT'] . "/escaperpg/aventures/ambria/includes/nav-sullivan.php"; ?>
			<div id="txt">
				...
				<?php elseif (isset($_POST['terreferme'])): ?>
					<?php $_SESSION['ile'] = true; ?>
					<audio src="/escaperpg/sons/ambria/plage.mp3" autoplay></audio>
					<p>
				...
				<?php elseif (isset($_POST['grottes'])): ?>
					<audio src="/escaperpg/sons/ambria/plage.mp3" autoplay></audio>
					<p>
				...
				<?php elseif (isset($_POST['4grotte'])): ?> <!-- bonne réponse -->
					<?php if (!isset($_SESSION['pertehomme'])): ?>
						<div id="succespopup">
							<?php
							$scenario = 'ambria';
							$nom = 'prudence';
				...
				<?php elseif (!empty(array_intersect(array_keys($_POST), ['1grotte', '2grotte', '3grotte', '5grotte', '6grotte']))): ?> <!-- mauvaise réponse, un mec meurt -->
					<?php if (isset($_POST['1grotte'])): ?>
						...
						<?php $_SESSION['sullivanconfiance'] -= 20; ?>
					<?php elseif (isset($_POST['2grotte'])): ?>
						...
						<?php $_SESSION['sullivanconfiance'] -= 20; ?>
					<?php elseif (isset($_POST['3grotte'])): ?>
						...
						<?php $_SESSION['sullivanconfiance'] -= 20; ?>
					<?php elseif (isset($_POST['5grotte'])): ?>
					...
					?>
				<?php elseif (isset($_POST['grottesenigme']) || isset($_SESSION['grottesenigme'])): ?>
					<audio src="/escaperpg/sons/ambria/plage.mp3" autoplay></audio>
					<div class="enigmelieu">
						<img src="/escaperpg/images/ambria/grottesentree.png" alt="entrées des grottes">
						<?php foreach (range(1, 6) as $i): ?>
						<div id="grotte<?= $i ?>">
							<form action="plage" method="post">
								<button type="submit" name="<?= $i ?>grotte">
									<img src="/escaperpg/images/ambria/grotte<?= $i ?>.png" alt="entrée de la grotte <?= $i ?>">
								</button>
							</form>
						</div>
						<?php endforeach; ?>
					...
					<p>
						...
						- Les portes s'ouvriront aux méritants, protégés de leur gardien volant."<br>
						<br>
						Quelle entrée devez-vous choisir ?
					</p>
					<?php
					$reponse = "Prenez le quatrième passage pour continuer votre chemin.";
					$indice1 = "Suivez bien les consignes énoncées par Jake et étudiez les statues pour savoir quelle entrée choisir.";
					$indice2 = "Procédez par élimination. Essayez de trouver les statues qui indiquent un passage mortel et celles auxquelles vous pouvez vous fier.";
					$indice3 = "Il vous faut trouver deux statues sûres entourant un passage. Vous pourrez alors y entrer. Plusieurs statues correspondent à cette description mais seul un passage est gardé par deux d'entre elles.";
