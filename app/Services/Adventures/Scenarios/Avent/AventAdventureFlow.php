...
namespace App\Services\Adventures\Scenarios\Avent;
...
use App\Services\Adventures\Scenarios\Avent\Scenes\AventSceneHandler;
use App\Services\Adventures\Scenarios\Avent\Scenes\FinSceneHandler;
use App\Services\Adventures\Support\AdventureSaveService;

class AventAdventureFlow extends SceneBasedAdventureFlow
{
    ...
    public function __construct()
    {
        ...
        foreach ($this->definitions() as $scene => $definition) {
            if ($scene === 'fin') {
                $this->handlers[$scene] = new FinSceneHandler((array) $definition);
                continue;
            }

            $this->handlers[$scene] = new AventSceneHandler(
                $scene,
    ...
    protected function handleGlobalAction(
        ...
        return match ($action) {
            'new_game' => new AdventureActionResult(
                nextScene: 'index',
                resetState: true,
                stateChanges: ['started' => true, 'index_variant' => 'introduction'],
                achievements: [
                    ['scenario' => 'general', 'name' => 'debut'],
                    ['scenario' => 'avent', 'name' => 'debut'],
                ],
            ),
    ...
    {
        return require dirname(__DIR__, 5) . '/config/adventures/avent/flow.php';
    }
}
