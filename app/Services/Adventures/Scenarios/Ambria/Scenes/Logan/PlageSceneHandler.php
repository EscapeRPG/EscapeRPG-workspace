...
class PlageSceneHandler extends AmbriaSceneHandler
{
    public function variant(AdventureState $state, Request $request, bool $isLandingPage = false): string
    {
        ...
        if ((bool) $state->get('matcasse', false) || (bool) $state->get('quillecassee', false)) {
            return 'damaged_arrival';
        }
    ...
    public function handle(array $config, AdventureState $state, Request $request): AdventureActionResult
    {
        ...
        return match ($action) {
            'continue_arrival' => new AdventureActionResult(
                nextScene: 'logan_plage',
                stateChanges: ['logan_plage_variant' => 'search_intro', 'ile' => true],
            ),
            'continue_grottes_intro' => new AdventureActionResult(nextScene: 'logan_plage', stateChanges: ['logan_plage_variant' => 'grottes_intro']),
            'continue_grottes_puzzle' => new AdventureActionResult(nextScene: 'logan_plage', stateChanges: ['logan_plage_variant' => 'grottes_puzzle', 'grottesenigme' => true]),
            'retry_grottes' => new AdventureActionResult(nextScene: 'logan_plage', stateChanges: ['logan_plage_variant' => 'grottes_puzzle', 'grottesenigme' => true]),
            'enter_grottes' => new AdventureActionResult(
                nextScene: 'logan_grottestorchesallumees',
                stateChanges: [
                    'logan_grottestorchesallumees_variant' => 'entry',
    ...
    private function handleGrotteChoice(AdventureState $state, int $grotte): AdventureActionResult
    {
        if ($grotte === 4) {
            return new AdventureActionResult(
                nextScene: 'logan_plage',
                stateChanges: [
                    'logan_plage_variant' => 'grotte_success',
                    'grottesenigme' => false,
                    'torcheseteintes' => false,
                ],
            );
        ...
        return new AdventureActionResult(
            nextScene: 'logan_plage',
            stateChanges: [
                'logan_plage_variant' => 'wrong_grotte_' . $grotte,
                'grottesenigme' => true,
                'loganconfiance' => (int) $state->get('loganconfiance', 0) - 20,
            ],
        );
    }
}
