...
const cartesRetournees = JSON.parse(container?.dataset.flippedCards || "[]");
...
for (let index = 1; index < 17; index++) {
  container.innerHTML += `
     <div class="carte-jeu" data-index="${index}">
        <img src="/assets/img/avent/cartes/${index}verso.png">
        <div class="hidden">
            <a href="/assets/img/avent/cartes/${index}recto.png" rel="lightbox[inventaire]">
                <img src="/assets/img/avent/cartes/${index}recto.png">
            </a>
        </div>
     </div>
    `;
...
function flipCard(carteVerso, carteRecto) {
  ...
  fetch("/aventures/avent/cartes", {
    method: "POST",
    headers: { "Content-Type": "application/x-www-form-urlencoded" },
    body: "action=flip_card&card_index=" + encodeURIComponent(index),
  });
}
