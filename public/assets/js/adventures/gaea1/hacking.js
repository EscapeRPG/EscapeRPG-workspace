...
const bulleMarv = document.querySelector(".bullemarv");
const terminal = document.querySelector("[data-terminal-wrap]");
let mappse = false,
  filse = false;

if (!consoleInput || !consoleOutput || !inputShadow || !bulleMarv || !terminal) {
  return;
}
  ...
  if (event.key === "Enter") {
    ...
    if (command) {
      consoleOutput.innerHTML +=
        '<span class="blue">SYSTEM</span> > <i>' + escapeHtml(command) + "</i>";
      this.value = "";
      consoleOutput.scrollTop = consoleOutput.scrollHeight;
      setTimeout(handleCommand, Math.floor(Math.random() * 600) + 500, command);
      adjustInputWidth(this.value);
    }
  }
});
...
- Passord : <span class="blue">hmpo62x4sdr</span>
  ...
  addNotes(["a_hyrd_xbp718", "hmpo62x4sdr"]);
}
...
function addNotes(items) {
  ...
  if (!notesListe) return;

  items.forEach((item) => {
    if (!Array.from(notesListe.options).some((option) => option.value === item)) {
