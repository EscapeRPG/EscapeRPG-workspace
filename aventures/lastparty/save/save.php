...
<html lang="fr">
	<head>
		...
		<meta charset="utf-8">
		<title>Sauvegarder - Last Party</title>
	</head>
			...
			<div id="txt">
				<?php
					if ($_SESSION['loggedin']) {
						...
						$page = $_SESSION['page'];
						$verifsave = $conn->prepare("SELECT * FROM lastparty WHERE id = ?");
						$verifsave->execute([$nom]);
						$saveexiste = $verifsave->rowCount();
						if ($saveexiste == 0) { $stmt = $conn->prepare("INSERT INTO lastparty (id, code, sess, savepage) VALUES (?, ?, ?, ?)"); $stmt->execute([$nom, $code, $session, $page]); }
						else { $stmt = $conn->prepare("UPDATE lastparty SET code=?, sess=?, savepage=? WHERE id=?"); $stmt->execute([$code, $session, $page, $nom]); }
