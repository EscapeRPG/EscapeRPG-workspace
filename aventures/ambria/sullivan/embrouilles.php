        ...
        <div id="txt">
            <?php if (isset($_POST['suite2'])): ?>
                ...
                <form action="embrouilles" method="post">
                    <input list="notesListe" name="danger">
            ...
            <?php elseif (isset($_POST['react'])): ?>
                <?php if (str_replace($search, $replace, stripslashes($_POST['danger'])) == "attention"): ?>
                    ...
                    <form action="embrouilles" method="post">
                        <input list="notesListe" name="bagarre">
                        <input type="submit" name="battre" value="Réagir.">
                    </form>
                    <?php if (!in_array('Derrière-toi', $_SESSION['mdp'])) {
                        $_SESSION['mdp'][] = "Derrière-toi";
                    }
                    ?>
                <?php else: ?>
                    <audio src="/escaperpg/sons/ambria/tavernehommetombe.mp3" autoplay></audio>
            ...
            <?php elseif (isset($_POST['battre'])): ?>
                <?php if (str_replace($search, $replace, stripslashes($_POST['bagarre'])) == "baissezvous"): ?>
                    ...
                    <form action="embrouilles" method="post">
                        <input list="notesListe" name="sortir">
                        <input type="submit" name="partir" value="Sortir.">
                    </form>
                    <?php
                    if (!in_array('On sort', $_SESSION['mdp'])) {
                        $_SESSION['mdp'][] = "On sort";
                    }
                    ?>
                <?php else: ?>
            ...
            <?php elseif (isset($_POST['loganreponse'])): ?>
                ...
                <?php else: ?>
                    ...
                    <form action="embrouilles" method="post">
                        <input list="notesListe" name="logannom">
                        <input type="submit" name="loganreponse" value="Suivant.">
                    </form>
                    <?php
                    $reponse = "La réponse est \"Logan\".";
                    $indice1 = "Vous avez demandé le nom de votre interlocuteur. Quel est-il ?";
                    $indice2 = "Il suffit de noter le prénom de ce personnage, incarné par le second joueur.";
                    $indice3 = "Demandez à votre partenaire le prénom de son personnage.";
                    include $_SERVER['DOCUMENT_ROOT'] . "/escaperpg/includes/indices.php";
                    ?>
                <?php endif; ?>
            <?php elseif (isset($_POST['seloigner']) || isset($_SESSION['seloigner'])): ?>
                <div class="dialogue">
                    <div class="portrait">
                        <img src="/escaperpg/images/ambria/loganbarthelemymini.png" alt="logan barthélémy">
                    </div>
