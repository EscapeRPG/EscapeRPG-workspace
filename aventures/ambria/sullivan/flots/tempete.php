    ...
    <main>
        ...
        <div id="txt">
            ...
            <?php elseif (isset($_POST['detour'])): ?>
                ...
                <?php
                if (!in_array('Barre à tribord', $_SESSION['mdp'])) {
                    $_SESSION['mdp'][] = 'Barre à tribord';
                }
                ?>
            <?php elseif (isset($_POST['enavant'])): ?>
                <audio src="/escaperpg/sons/ambria/ordres.mp3" autoplay></audio>
                <p>
                ...
                if (!in_array('Branle-bas', $_SESSION['mdp'])) {
                    $_SESSION['mdp'][] = 'Branle-bas';
                }
                ?>
            ...
            <?php elseif (isset($_POST['affaler'])): ?>
                ...
                if (!in_array('Affale', $_SESSION['mdp'])) {
                    $_SESSION['mdp'][] = 'Affale';
                }
                ?>
            <?php elseif (isset($_POST['ferler'])): ?>
                <audio src="/escaperpg/sons/ambria/tempetecracks.mp3" autoplay></audio>
                <div class="dialogue">
                    <div class="bulleperso2">
                        <p>
                ...
                <?php
                if (!in_array('Ferle', $_SESSION['mdp'])) {
                    $_SESSION['mdp'][] = 'Ferle';
                }
                ?>
            <?php elseif (isset($_POST['vague'])): ?>
                <?php switch (str_replace($search, $replace, stripslashes($_POST['vigie']))):
                    case "scelerateababord": ?>
                        ...
                        if (!in_array('Les tonneaux', $_SESSION['mdp'])) {
                            $_SESSION['mdp'][] = 'Les tonneaux';
                        }
                        ?>
                    <?php break;
                    case "scelerateatribord": ?>
                        <audio src="/escaperpg/sons/ambria/scelerate.mp3" autoplay></audio>
                        <p>
                    ...
                    default: ?>
                        ...
                        ?>
                <?php endswitch; ?>
            <?php elseif (isset($_POST['poursuivre'])): ?>
                <audio src="/escaperpg/sons/ambria/tempete.mp3" autoplay></audio>
                <p>
            ...
            <?php elseif (isset($_POST['recifs']) || isset($_SESSION['recifs'])): ?>
                <?php if (isset($_POST['1barre']) || isset($_POST['2barre']) || isset($_POST['3barre']) || isset($_POST['4barre']) || isset($_POST['5barre'])): ?>
                    <?php if (isset($_SESSION['recifsetape4'])): ?>
                        <?php
                        unset($_SESSION['recifs']);
                        $_SESSION['recifsetape5'] = true;
                        ?>
                        <?php if (isset($_POST['4barre'])): ?>
                        ...
                        <?php else: ?>
                            ...
                            <?php $_SESSION['etatquille'] -= 50; ?>
                        <?php endif; ?>
                    <?php elseif (isset($_SESSION['recifsetape3'])): ?>
                        <?php if (isset($_POST['3barre'])): ?>
                            <audio src="/escaperpg/sons/ambria/recifevite.mp3" autoplay></audio>
                            <p>
                        ...
                        <?php else: ?>
                            ...
                            <?php $_SESSION['etatquille'] -= 50; ?>
                        <?php endif; ?>
                        <?php $_SESSION['recifsetape4'] = true; ?>
                    <?php elseif (isset($_SESSION['recifsetape2'])): ?>
                        <?php if (isset($_POST['5barre'])): ?>
                            <audio src="/escaperpg/sons/ambria/recifevite.mp3" autoplay></audio>
                            <p>
                        ...
                        ?>
                    <?php elseif (isset($_SESSION['recifs'])): ?>
                        <?php if (isset($_POST['2barre'])): ?>
                            <audio src="/escaperpg/sons/ambria/recifevite.mp3" autoplay></audio>
                            <p>
                ...
                <?php if (!isset($_SESSION['recifsetape5'])): ?>
                    <div id="enigmelieu">
                        ...
                        <form action="tempete" method="post">
                            ...
                            <button name="2barre" id="barre2"></button>
                            <button name="3barre" id="barre3"></button>
                            <button name="4barre" id="barre4"></button>
                            <button name="5barre" id="barre5"></button>
                        </form>
                    </div>
                    <?php
                    $_SESSION['recifs'] = true;

                    $reponse = "Pour le premier récif, cliquez sur la 2e poignée.<br>Pour le second récif, cliquez sur la 5e poignée.<br>Pour le troisième récif, cliquez sur la 3e poignée.<br>Pour le dernier récif, cliquez sur la 4e poignée.";
                    $indice1 = "Le journal de bord que vous avez récupéré dans votre cabine vous sera très utile. Vous devriez regarder dans votre inventaire.";
                    $indice2 = "Logan doit vous dire ce qu'il voit.<br>Demandez-lui de bien décrire la forme et la position des récifs pour vous permettre de savoir quel chemin emprunter. Essayez tous les deux de vous représenter la scène en 5 cases, correspondant à chacune des poignées de la barre.";
                    $indice3 = "Les deux premiers récifs correspondent à la figure 1, vous devez passer le plus près possible.<br>Le troisième récif correspond à la figure 4, passez une nouvelle fois au plus près.<br>Le dernier récif correspond à la figure 2, il faut donc passer deux cases plus loin.";
            ...
            <?php elseif (isset($_POST['tempetefin'])): ?>
                ...
                <?php if (isset($_SESSION['matcasse']) && isset($_SESSION['quillecassee'])): ?>
                    ...
                    <?php
                    if (!in_array('Sortez les rames', $_SESSION['mdp'])) {
                        $_SESSION['mdp'][] = 'Sortez les rames';
                    }
                    ?>
                <?php elseif (isset($_SESSION['matcasse'])): ?>
                    <audio src="/escaperpg/sons/ambria/tempetefinmatcasse.mp3" autoplay></audio>
                    <p>
                        Alors que le danger des récifs semble être écarté, une nouvelle rafale vient ébranler le navire.<br>
                        Après un énorme craquement sinistre, vous voyez le mât principal commencer à s'écrouler.
                    ...
                    <?php
                    if (!in_array('Rames', $_SESSION['mdp'])) {
                        $_SESSION['mdp'][] = 'Rames';
                    }
                    ?>
                <?php elseif (isset($_SESSION['quillecassee'])): ?>
                    ...
                    <?php
                    if (!in_array('Accoster', $_SESSION['mdp'])) {
                        $_SESSION['mdp'][] = 'Accoster';
                    }
                    ?>
                <?php else: ?>
                    <div id="succespopup">
                    ...
                    <?php
                    if (!in_array('Chaloupe', $_SESSION['mdp'])) {
                        $_SESSION['mdp'][] = 'Chaloupe';
                    }
                    ?>
                <?php endif; ?>
            <?php else: ?>
                ...
                <?php if (!isset($_SESSION['ambriajournalsullivan'])): ?>
                    <p>
                ...
                <?php
                if (!in_array('Tout le monde à son poste', $_SESSION['mdp'])) {
                    $_SESSION['mdp'][] = 'Tout le monde à son poste';
                }
                ?>
            <?php endif; ?>
        </div>
