...
<html lang="fr">
	<head>
		...
		<meta charset="utf-8">
		<title>Charger - Le Trésor d'Ambria</title>
	</head>
			...
			<div id="txt">
				<?php
					if ($_SESSION['loggedin']) {
						...
						$query = $conn->prepare("SELECT * FROM ambria WHERE id=? ORDER BY id DESC LIMIT 1");
						$query->execute([$nom]);
						$result = $query->fetch();
						if ($result) {
							$sess = $result['sess'];
					...
					elseif (isset($_POST['continuer'])) {
						...
						$nom = $_POST['nom'];
						$code = $_POST['code'];
						$query = $conn->prepare("SELECT * FROM ambria WHERE id=? AND code=?");
