			...
			<div id="txt">
				<?php if ($_SESSION['loggedin']): ?>
					...
					<?php
						...
						if ($saveexiste == 0) { $stmt = $conn->prepare("INSERT INTO gaea1 (id, code, sess, savepage) VALUES (?, ?, ?, ?)"); $stmt->execute([$nom, $code, $session, $page]); }

						else { $stmt = $conn->prepare("UPDATE gaea1 SET code=?, sess=?, savepage=? WHERE id=?"); $stmt->execute([$code, $session, $page, $nom]); }
					?>
