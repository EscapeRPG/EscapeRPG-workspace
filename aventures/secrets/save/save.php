			...
			<div id="txt">
				<?php
					if ($_SESSION['loggedin']) {
						...
						$page = $_SESSION['page'];
						$verifsave = $conn->prepare("SELECT * FROM secretsfamiliaux WHERE id = ?");
						$verifsave->execute([$nom]);
						$saveexiste = $verifsave->rowCount();
						if ($saveexiste == 0) { $stmt = $conn->prepare("INSERT INTO secretsfamiliaux (id, code, sess, savepage) VALUES (?, ?, ?, ?)"); $stmt->execute([$nom, $code, $session, $page]); }
						else { $stmt = $conn->prepare("UPDATE secretsfamiliaux SET code=?, sess=?, savepage=? WHERE id=?"); $stmt->execute([$code, $session, $page, $nom]); }
